    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch in self.valid_iter:
                source = batch.eng # 한국어가 아닌 영어를 입력으로 넣는다.
                target = batch.kor # 영어가 아닌 한국어를 출력값으로 설정한다.
    ...
    def inference(self):
        ...
        with torch.no_grad():
            for batch in self.test_iter:
                source = batch.eng # English를 Korean으로 번역해야하므로, input source를 eng로 지정합니다.
                target = batch.kor # target label은 kor로 지정합니다.
