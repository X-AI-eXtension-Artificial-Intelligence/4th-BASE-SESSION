...
class MultiHeadAttention(nn.Module):
    def __init__(self, params):
        ...
        self.attentions = nn.ModuleList([SelfAttention(params)
                                         for _ in range(params.n_head)]) #selfattention을 n_head만큼 반복해서 attntions 생성
...
class SelfAttention(nn.Module):
    def __init__(self, params):
        ...
        self.q_w = nn.Linear(self.hidden_dim, self.attention_dim, bias=False) #Q,K,V 차원 맞추기
        ...
        self.dropout = nn.Dropout(params.dropout)
        self.scale_factor = torch.sqrt(torch.FloatTensor([self.attention_dim])).to(params.device) #텐서 생성 = dk 제곱근

    def forward(self, query, key, value, mask=None):
        # query, key, value = [batch size, sentence length, hidden dim]

        # create Q, K, V matrices using identical input sentence to calculate self-attention score
        q = self.q_w(query) # q,k,v 벡터 생성
        k = self.k_w(key)
        v = self.v_w(value)
        # q, k, v = [batch size, sentence length, attention dim]

        self_attention = torch.bmm(q, k.permute(0, 2, 1)) #q, k 내적
        self_attention = self_attention / self.scale_factor # attetnion 값 생성
        # self_attention = [batch size, sentence length, sentence length]

        if mask is not None:
            self_attention = self_attention.masked_fill(mask, -np.inf) #마스크 진행

        # normalize self attention score by applying soft max function on each row
        attention_score = F.softmax(self_attention, dim=-1) #소프트맥스 진행
        ...
        weighted_v = torch.bmm(norm_attention_score, v) #v랑 내적 진행 -> weighted_v가 attention matrix
