...
def clean_text(text):
    ...
    Returns:
        normalized sentence
    """
    ## 특수 문자들을 정규식 패턴으로 찾아내어 빈 문자열로 치환환
    text = re.sub('[-=+,#/\?:^$.@*\"※~&%ㆍ!』\\‘|\(\)\[\]\<\>`…》]', '', text)
    return text

## dataframe으로부터 'torchtext' dataset을 생성하기 위한 전처리 함수
def convert_to_dataset(data, kor, eng):
    ...
    ## dataframe에서 str 타입이 아닌 데이터를 가진 row 제거
    ...
    ## 각 row를 'kor'와 'eng' Fiels를 가지는 'Example'로 변환환
    ...
    ## torchtext 'Example' 리스트를 이용하여 torchtext 'Dataset' 생성성
...
class Params:
    ...
    def load_vocab(self):
        ...
        ## 하이퍼파라미터와 vocab 정보를 params 딕셔너리에 할당
        params = {'input_dim': len(eng.vocab), 'output_dim': len(kor.vocab), #!!
                  'sos_idx': kor.vocab.stoi['<sos>'], 'eos_idx': kor.vocab.stoi['<eos>'], #!! ##decoder에서 사용하는 <sos>, <eos> 토큰 index
                  'pad_idx': kor.vocab.stoi['<pad>'], 'device': device} #!! ## <pad> 토큰 index

        self.__dict__.update(params) ## params 딕셔너리를 현재 인스턴스에 업데이트
