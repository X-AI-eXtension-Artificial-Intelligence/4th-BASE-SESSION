...
class DecoderLayer(nn.Module):
    ...
    def forward(self, target, encoder_output, target_mask, dec_enc_mask):
        # target          = [batch size, target length, hidden dim] ## decoder의 입력
        ...
        norm_target = self.layer_norm(target) ## 정규화
        output = target + self.self_attention(norm_target, norm_target, norm_target, target_mask)[0] ## self-attention 수행 후 더해줌
        ...
        sub_layer, attn_map = self.encoder_attention(norm_output, encoder_output, encoder_output, dec_enc_mask) ## encoder-decoder attention
        output = output + sub_layer ## residual connection

        norm_output = self.layer_norm(output) ## 정규화
        output = output + self.position_wise_ffn(norm_output) ## feedforward layer 통과 후 residual connection
...
class Decoder(nn.Module):
    ...
    def forward(self, target, source, encoder_output):
        ...
        ## target 텐서를 embedding하고 scaling값 곱해줌
        target = self.token_embedding(target) * self.embedding_scale
        ## positional encoding이 적용된 결과를 dropout하여 반환
