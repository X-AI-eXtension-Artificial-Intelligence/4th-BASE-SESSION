...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    # build_message에서 만든 텍스트 및 레이블 필드를 로드합니다

    file_kor = open('pickles/kor.pickle', 'rb') # 한국 pickle
    kor = pickle.load(file_kor)

    file_eng = open('pickles/eng.pickle', 'rb') # 영어 pickle
    ...
    if mode == 'train':
        train_data = convert_to_dataset(train_data, kor, eng) # 위 함수 
        valid_data = convert_to_dataset(valid_data, kor, eng) # 위 함수

        # make iterator using train and validation dataset
        print(f'Make Iterators for training . . .')
        '''
        Pytorch의 dataloader와 비슷한 역할을 한다.
