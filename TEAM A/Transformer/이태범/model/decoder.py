...
class DecoderLayer(nn.Module):
    ...
    def forward(self, target, encoder_output, target_mask, dec_enc_mask):
        ...
        # key, value를 encoder의 output으로 사용, dec_enc_mask는 왜? 
        sub_layer, attn_map = self.encoder_attention(norm_output, encoder_output, encoder_output, dec_enc_mask)
        output = output + sub_layer # summation? # add

        norm_output = self.layer_norm(output) # norm
        # feed forward?
        output = output + self.position_wise_ffn(norm_output) # position을 여기서?
...
class Decoder(nn.Module):
    ...
    def forward(self, target, source, encoder_output):
        ...
        # encoder_output      = [batch size, source length, hidden dim]
        target_mask, dec_enc_mask = create_target_mask(source, target) # 여길 이해해야되는데..!
        # source와 target으로 뭐가 들어간다는 건가
