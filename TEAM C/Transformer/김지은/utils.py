...
def convert_to_dataset(data, kor, eng): # 
    ...
    missing_rows = [idx for idx, row in data.iterrows() if type(row.korean) != str or type(row.english) != str] #문자열이 아닌애들을 missing row에 집어넣음
    #data의 각 행을 반복하면서 korean과 english이 모두 문자열이 아닌 경우, 누락된 행을 나타내는 인덱스를 저장장
    data = data.drop(missing_rows) #누락된 행들 지워라
    ...
    #각 데이터 행을 반복하면서 clean text 함수 적용
    #examplefromlist를 활용해 kor과 eng 필드와 함께 example 객체로 변환
    #kor 필드에는 한국어 문장을 위한 필드 객체, eng 필드에는 영어 문장을 위한 field 객체를 전달함
...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    if mode == 'train':
        ...
        train_iter, valid_iter = ttd.BucketIterator.splits(
            ...
            sort_key=lambda sent: len(sent.eng), #바꿈
            #iterator에서 반복되는데 사용되는 배치의 순서를 정할 수 있음
            # len(eng)를 통해 batch에서 영어 문장 길이에 따라 정렬함
            # 길이가 짧은 문장 순서대로 정렬함?
