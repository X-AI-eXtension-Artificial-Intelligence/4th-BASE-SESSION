...
class PositionWiseFeedForward(nn.Module): # encoder와 decoder의 각각의 층에서 사용됨
    ...
    def forward(self, x):
        ...
        output = self.dropout(F.relu(self.conv1(x)))  # output = [batch size, feed forward dim, sentence length) #Relu 활성화 함수 사용
