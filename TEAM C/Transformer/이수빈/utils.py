...
def load_dataset(mode):
    ...
    print(f'Loading Eng-Kor translation dataset and converting it to pandas DataFrame...')

    data_dir = Path().cwd() / 'data'
    # 현재 작업중인 경로 + /data
    # ex. cwd() = /User/2soup, cwd()/'data' = /User/2soup/data

    if mode == 'train':
        train_file = os.path.join(data_dir, 'train.csv') # 설정 경로에서 train.csv 경로
        train_data = pd.read_csv(train_file, encoding='utf-8') # train.csv파일 불러오기
...
def clean_text(text): # 문장 내 특수문자 처리
    """
...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    if mode == 'train':
        ...
        #train_data = convert_to_dataset(train_data, eng, kor)
        #valid_data = convert_to_dataset(valid_data, eng, kor)
        ...
        #test_data = convert_to_dataset(test_data, eng, kor)
