...
class MultiHeadAttention(nn.Module):
    def __init__(self, params):
        ...
        assert params.hidden_dim % params.n_head == 0 # assert 해당 조건이 틀리면 에러나지롱
        ...
        # nn.Linear(input sample size, output sample size, bias=False)
        # bias=False : bias 학습하지않는다.
    ...
    def forward(self, query, key, value, mask=None):
        ...
        output = self.dropout(self.o_w(weighted_v)) # dropout 비율
