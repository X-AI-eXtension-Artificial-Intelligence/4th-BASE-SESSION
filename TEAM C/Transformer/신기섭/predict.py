...
def predict(config):
    ...
    model.load_state_dict(torch.load(params.pre_trained_model))
    model.to(params.device)
    model.eval()

    # convert input into tensor and forward it through selected model
    tokenized = spacy_en(input)
    indexed = [eng.vocab.stoi[token.text] for token in tokenized]
    ...
    eos_idx = int(torch.where(target[0] == eos_idx)[0][0]) # or any other appropriate value
