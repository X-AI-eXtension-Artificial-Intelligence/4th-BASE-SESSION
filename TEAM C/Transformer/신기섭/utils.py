...
def load_dataset(mode):
    ...
    """
    print(f'Loading AI Hub Eng-Kor translation dataset and converting it to pandas DataFrame . . .')
...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    kor_df = pickle.load(file_kor)
    # 데이터프레임을 Vocab 객체로 변환
    kor_counter = Counter([i.word for i in kor_df.itertuples() for _ in range(i.index)])
    kor_obj = vocab.Vocab(kor_counter)

    # Field 객체에 Vocab 객체 적용
    okt = Okt()
...
def epoch_time(start_time, end_time): # trainer에서 호출되는 함수
    """
    Calculate the time spent to train one epoch  # 한 에폭당 시간이 얼마나 걸리는지 계산해주는 함수
    Args:
        start_time: (float) training start time  # 시작 시간
        end_time: (float) training end time      # 끝나는 시간

    Returns:
        (int) elapsed_mins and elapsed_sec spent for one epoch # 한 에폭에 걸린 분, 초를 return
    """
...
def display_attention(candidate, translation, attention):
    ...
    ax.set_xticklabels([''] + [t.text.lower() for t in candidate], rotation=45, fontproperties=fontprop)
...
class Params:
    ...
    def load_vocab(self):
        ...
        params = {'input_dim': len(eng.vocab), 'output_dim': len(kor.vocab),
                  'sos_idx': kor.vocab.stoi['<sos>'], 'eos_idx': kor.vocab.stoi['<eos>'],
                  'pad_idx': kor.vocab.stoi['<pad>'], 'device': device}
