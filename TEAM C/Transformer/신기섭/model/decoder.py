...
from model.attention import MultiHeadAttention # model 폴더 -> attention -> MultiHeadAttention을 불러온다.
from model.positionwise import PositionWiseFeedForward # model 폴더 -> positionwise -> PositionWiseFeedForward을 불러온다.
...
class DecoderLayer(nn.Module):
    def __init__(self, params, pre_trained=False):
        ...
        self.encoder_attention = MultiHeadAttention(params, pre_trained=self.pre_trained)
...
class Decoder(nn.Module):
    def __init__(self, params, pre_trained=False):
        ...
        self.decoder_layers = nn.ModuleList([DecoderLayer(params, pre_trained=self.pre_trained) for _ in range(params.n_layer)])
