...
# if encounters non-text row, we should skip it
    ...
    '''eng_lines = [row.english
                 for _, row in df.iterrows() if type(row.english) == str] 변환하려다가 안함''' 

    word_extractor = WordExtractor(min_frequency=5)
    '''word_extractor.train(eng_lines) 변환하려다가 안함'''
...
def build_vocab(config):
    ...
    kor = ttd.Field(tokenize=tokenizer.tokenize,
                    init_token='<sos>', ## sos 추가 (eng->kor)
                    eos_token='<eos>', ## eos 추가 (eng->kor)
                    lower=True,
                    batch_first=True)
