...
class MultiHeadAttention(nn.Module):
    def __init__(self, params):
        ...
        self.attentions = nn.ModuleList([SelfAttention(params) for _ in range(params.n_head)]) #n_head = attention 레이어 헤드 개수 (병렬 처리)
        self.o_w = nn.Linear(params.hidden_dim, params.hidden_dim, bias=False) # 선형 결합 (linear 함수)
        init_weight(self.o_w) # 가중치 초기화
        self.dropout = nn.Dropout(params.dropout) # 드롭아웃 

    def forward(self, query, key, value, mask=None): # 순전파 진행을 위해 이전 레이어에서 query, key, value
        # query, key, value = [batch size, sentence length, hidden dim]

        self_attentions = [attention(query, key, value, mask) for attention in self.attentions] # 병렬처리 할 수 있도록 리스트 생성
        ...
        weighted_v = torch.cat(weighted_vs, dim=-1) #concat으로 쌓는다
        # weighted_v = [batch size, sentence length, hidden dim]

        output = self.dropout(self.o_w(weighted_v)) #드롭아웃
        # output = [batch size, sentence length, hidden dim]

        return output, attentions


class SelfAttention(nn.Module): # dot.product (점곱 어텐션)
    def __init__(self, params):
        super(SelfAttention, self).__init__()
        self.hidden_dim = params.hidden_dim
        self.attention_dim = params.hidden_dim // params.n_head

        self.q_w = nn.Linear(self.hidden_dim, self.attention_dim, bias=False) #q, k, v 각각 선형 변환
        ...
        self.dropout = nn.Dropout(params.dropout) #서브 레이어 이후의 dropout
        self.scale_factor = torch.sqrt(torch.FloatTensor([self.attention_dim])).to(params.device) #스케일링
    ...
    def forward(self, query, key, value, mask=None):
        ...
        weighted_v = torch.bmm(norm_attention_score, v) # 가중치 업데이트???
