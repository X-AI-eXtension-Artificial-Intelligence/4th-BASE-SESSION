...
class EncoderLayer(nn.Module): # Encoder의 레이어. multi-head attention과 Position-wise feed-forward로 이루어져 있음
    ...
    def forward(self, source, source_mask): # 순전파
        # source          = [batch size, source length, hidden dim]
        # source_mask     = [batch size, source length, source length]

        # Original Implementation: LayerNorm(x + SubLayer(x)) -> Updated Implementation: x + SubLayer(LayerNorm(x)) # 그렇다고 하네요...
...
class Encoder(nn.Module):
    def __init__(self, params):
        ...
        self.token_embedding = nn.Embedding(params.input_dim, params.hidden_dim, padding_idx=params.pad_idx) #토큰 임베딩
        ...
        self.pos_embedding = nn.Embedding.from_pretrained(
            create_positional_encoding(params.max_len+1, params.hidden_dim), freeze=True) #positional encoding (위치 인코딩)

        self.encoder_layers = nn.ModuleList([EncoderLayer(params) for _ in range(params.n_layer)]) # encoding layer를 쌓는다
