...
def load_dataset(mode):
    ...
    data_dir = Path().cwd() / 'data'              # 현재 작업 directory/data

    if mode == 'train':                                                        
        train_file = os.path.join(data_dir, 'train.csv')           # train과 valid data 파일 read
        ...
        test_file = os.path.join(data_dir, 'test.csv')             # test data 파일 read
...
def clean_text(text):
    ...
    Returns:
        normalized sentence
    """
    text = re.sub('[-=+,#/\?:^$.@*\"※~&%ㆍ!』\\‘|\(\)\[\]\<\>`…》]', '', text)  # 특수문자 제거로 문장의 일반화
    return text
...
def make_iter(batch_size, mode, train_data=None, valid_data=None, test_data=None):
    ...
    """
    # build_pickles.py로 만든 text와 label field를 load
    ...
    if mode == 'train':                                                  # mode 지정 후  
        ...
        train_iter, valid_iter = ttd.BucketIterator.splits(
            ...
            # 버킷 반복기는 데이터를 그룹화하는 데 어떤 기능을 사용해야 하는지 알려주어야 함
            # 이 경우 예제 텍스트를 사용하여 데이터 집합을 정렬
            sort_key=lambda sent: len(sent.kor),
            # 모든 텐서는 아래 옵션에 따라 길이별로 정렬
...
class Params:
    ...
    def update(self, json_path):
        """json_path에 있는 json파일 내부의 인자들을 load 후"""
        with open(json_path) as f:            
            params = json.load(f)
            self.__dict__.update(params)

    def load_vocab(self):
        # kor 및 eng vocab을 로드하여 vocab 크기 구성 추가
